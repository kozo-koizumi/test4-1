...
if mode == "採寸入力":
    ...
    if st.session_state.edit_order:
        ...
        # 注文内容サマリー
        #summary = [f"・{spec.get('label', k)}: {order.get(k, 0)}点" for k, spec in product_specs.items() if order.get(k, 0) > 0]
        #if summary:
        #    with st.expander("📝 注文内容の詳細", expanded=True):
        ...
        for key, spec in product_specs.items():
            ...
            with st.container(border=True):
                ...
                if st.button(f"{display_name} を一時保存", key=f"btn_{key}"):
                    ...
                    st.toast(f"{display_name} を保存しました")
